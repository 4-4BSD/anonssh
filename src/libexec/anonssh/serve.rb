def main(argv)
  name, path = AnonSSH.parse(:serve, argv)
  if name.nil? || path.nil?
    AnonSSH.error! "serve requires -n NAME and -p PATH"
  elsif !File.exist?(path)
    AnonSSH.error! "#{path} does not exist"
  end

  command = AnonSSH.mount_devfs(path)
  if command.failure?
    AnonSSH.error!(command.stderr)
  end

  command = AnonSSH.devfs_ruleset(path)
  if command.failure?
    AnonSSH.error!(command.stderr)
  end

  command = AnonSSH.devfs_applyset(path)
  if command.failure?
    AnonSSH.error!(command.stderr)
  end

  jail = Jail.create(name:,path:,ip4: "inherit",devfs_ruleset: 4)
  jail.attach
  pid = Process.spawn("/usr/sbin/sshd", "-D", "-e", "-f", "/etc/ssh/sshd_config")
  Process.waitpid(pid)
end
main(ARGV)
